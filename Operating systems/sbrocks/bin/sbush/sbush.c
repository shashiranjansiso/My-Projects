...
/* 	if(strcmp("kill", cmd) == 0)
    {
				printf("In kill sbush \n");
        if(commands[0][1] ==NULL)
				{
					printf("kill: Too few arguments. \n");
					exit(1);
				}
				int pid = atoi(commands[0][1]);
...
void executeProcess(char *envp[]){
    ...
    {
				//printf("command not found block \n");
