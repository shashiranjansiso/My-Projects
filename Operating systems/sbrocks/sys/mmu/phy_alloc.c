...
static uint64_t refcount[(PAGE_MAX/64) +1][64];
...
uint64_t get_ref_count(uint64_t phy)
{
	uint64_t page = mm_phy_to_page(phy);
	if(page)
	{
		//kprintf2("ref count in get_ref_count: %d \n", refcount[page/64][page%64]);
		return refcount[page/64][page%64];
	}
	return -1;
}
