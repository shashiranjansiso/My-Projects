...
uint64_t pagetablentry = 0x1;
uint64_t start_kern_phy, end_kern_phy;
uint64_t kernel_cr3;
uint64_t is_user_page(uint64_t pagetableentry);
//TODO: Change it using self referencing table 

void set_start(uint64_t start)
{
	start_kern_phy = start;
}
...
{
	//pagetablentry = (uint64_t *)0xFFFFFFFF800B8000;
...
void free_page_tables(uint64_t pml4e){
	...
	//mark_kernel_pages(start_kern_phy, end_kern_phy);
	//kprintf2("Done \n");
}
//}
...
void set_writable_bit(uint64_t *pagetableentry)
{
	pagetablentry = pagetablentry | 0x02UL;	
}

void reset_writable_bit(uint64_t *pagetableentry)
{
	pagetablentry = pagetablentry & 0xFFFFFFFFFFFFFFFDUL;	
}

void set_cow_bit(uint64_t *pagetableentry)
{
	pagetablentry = pagetablentry | 0x4000000000000000UL;	
}

void reset_cow_bit(uint64_t *pagetableentry)
{
	pagetablentry = pagetablentry & 0xBFFFFFFFFFFFFFFFUL;	
}

uint64_t is_cow_set(uint64_t *pagetableentry)
{
	return *pagetableentry & 0x4000000000000000UL;
}

uint64_t is_writable_page(uint64_t *pagetableentry)
{
	return *pagetableentry & 0x02;
}

uint64_t is_user_page(uint64_t pagetableentry)
{
	return pagetableentry & 0x04;	
}