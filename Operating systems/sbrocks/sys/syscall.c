...
/* 	kstrcpy(envp[0],"hjfer");
	...
	//kprintf2("process_name: %s\n",t->name);
	if(t!= NULL)
	{
		free_task_struct(t);
		t->state = TASK_STOPPED;
		//kprintf2("After stopped \n");
		return 0;
	}
	return -1;
}
...
const char *task_state[10]= {"RUNNING","IDLE","INTERRUPTIBLE","UNINTERRUPTIBLE","ZOMBIE","STOPPED","SWAPPING","RUNNABLE","SLEEP","WAITING"};

void system_listprocess()
{
	struct task_struct *t = get_task_list();
...
uint64_t system_fork(){
	...
	if(child == NULL)
	{	
		//kprintf2("max process limit exceeded .. please kill some running tasks \n");
		return -1;
	}
	...
	while(vm_list != NULL)
	{
		...
		else
		{
			...
			while(vaddr < end)
			{
				...
				//kprintf2("between \n\n");
				inc_ref_count(paddr);
				//kprintf2("phy addr: %x , ref count is : %d \n", paddr, get_ref_count(paddr));
				//kprintf2("after \n\n");
				//pte = get_pte_entry(vaddr);
...
/* 	envp[0] = ((char **)envp_addr) [0];
	...
	*/
	...
	if(argv_addr == 0)
	{
		...
		envp[1] = "shashi";
		task = readElf(fname, NULL, i, envp, 1);
		if(task == NULL)
		{
			kprintf2("max process limit exceeded .. please kill some running tasks \n");
			return;
	...
	{
		task = readElf(fname, argv, i, envp, 1);
		if(task == NULL)
		{
			kprintf2("max process limit exceeded .. please kill some running tasks \n");
			return;
